    ...
    function register_post_types(){
      register_post_type( 'lesson', [
        ...
        'labels' => [
          ...
          'add_new_item'       => __('Added lesson', 'universal'), // заголовка у вновь создаваемой записи в админ-панели.
          'edit_item'          => __('Edit lesson', 'universal'), // для редактирования типа записи
          'new_item'           => __('New lesson', 'universal'), // текст новой записи
          'view_item'          => __('View lessons', 'universal'), // для просмотра записи этого типа.
          ...
          'menu_name'          => __('Lesson', 'universal'), // название меню
        ],
...
function universal_theme_widgets_init() {
  register_sidebar(
    array(
      'name'          => esc_html__( 'Sidebar on main', 'universal' ),
      'id'            => 'main-sidebar',
  ...
  register_sidebar(
    ...
    register_sidebar(
    array(
      'name'          => esc_html__( 'Recommended articles', 'universal' ),
      'id'            => 'sidebar-articles',
      'description'   => esc_html__( 'Add widgets here.', 'universal' ),
      'before_widget' => '<section id="%1$s" class="widget %2$s">',
      'after_widget'  => '</section>',
...
class Downloader_Widget extends WP_Widget {
  ...
  */
  function form( $instance ) {
    ...
    <p>
      <label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e('Link:', 'universal'); ?></label>
...
class Social_Widget extends WP_Widget {
  ...
  */
  ...
  <p>
    <label for="<?php echo $this->get_field_id( 'vkontakte' ); ?>"><?php _e('VKontakte:', 'universal'); ?></label>
...
class Recent_Post_Widget extends WP_Widget {
  ...
  */
  ...
  // фильтр чтобы можно было отключить скрипты
    if( ! apply_filters( 'show_recent_post_widget_script', true, $this->id_base ) ) return;
...
class Posts_Widget extends WP_Widget {
  ...
  */
    function form( $instance ) {
      $title = @ $instance['title'] ?: __('Recommended articles', 'universal');
